...
import '../Service/Firebase/Utils.dart';
...
class _ProfileState extends State<Profile> {
    ...
    return Scaffold(
      ...
      body: Center(
        child: SingleChildScrollView(
          child: isLoading ? buildLoadingScreen(context)
              :Column(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              Stack(
                children: [
                  !currentUser.imgPath.contains("http") && !currentUser.imgPath.contains("images")
                      ? CircleAvatar(
                    radius: 60,
                    backgroundImage: MemoryImage(convertStringToUint8List(currentUser.imgPath)),
                  )
                      : CircleAvatar(
                    radius: 60,
                    child: ClipOval(
                      ...
                      child: Form(
                        ...
                        child: Column(
                          children: [
                            ...
                            ggCheck ? Container() : GestureDetector(
                              onTap: (){
            ...
            TextButton(
              ...
              onPressed: () async{
                if(_formkey.currentState!.validate()){
                  UserDetail tmp = UserDetail(fullName: nameController.text, userName: nameController.text, email: currentUser.email.toString(), imgPath: currentUser.imgPath, id: currentUser.id);
                  await databaseRef.updateUserDetail(currentUser.id, tmp);
                  await userDatabase.update(id: currentUser.id, fullName: nameController.text, userName: nameController.text, email: currentUser.email.toString(), password: currentUser.password??'', imgPath: currentUser.imgPath);
                  var dataUser=await userDatabase.fetchById(currentUser.id);
                  await prefs.setString("user",jsonEncode(dataUser.toJson()));
                  getSharedPreferences();
                  ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                      backgroundColor: Colors.blue,
                      content: Text("Thay đổi tên tài khoản thành công!",)));
                  setState(() {
                    getSharedPreferences();
                  });
                }
        ...
        return AlertDialog(
          ...
          actions: <Widget>[
            ...
            TextButton(
              child: const Text('Lưu'),
              onPressed: () async{
                if(_formkey.currentState!.validate() && oldPassController.text == password){
                  changePassword(email: currentUser.email, oldPassword: oldPassController.text, newPassword: newPassController.text);
                  await userDatabase.update(id: currentUser.id, fullName: nameController.text, userName: nameController.text, email: currentUser.email.toString(), password: newPassController.text??'', imgPath: currentUser.imgPath);
                  var dataUser=await userDatabase.fetchById(currentUser.id);
                  await prefs.setString("user",jsonEncode(dataUser.toJson()));
  ...
  void changePassword({required email, required oldPassword, required newPassword}) async{
  ...
  void selectImage() async{
    ...
    setState(() {
      _image = img;
      UserDetail tmp = UserDetail(fullName: nameController.text, userName: currentUser.email.toString().substring(0,currentUser.email.toString().indexOf('@')), email: currentUser.email, imgPath: convertUint8ListToString(_image!), id:currentUser.id);
      databaseRef.updateUserDetail(currentUser.id, tmp);
    });
    await userDatabase.update(id: currentUser.id, fullName: nameController.text, userName: currentUser.email.toString().substring(0,currentUser.email.toString().indexOf('@')), email: currentUser.email, password: currentUser.password??'', imgPath: convertUint8ListToString(_image!));
    var dataUser=await userDatabase.fetchById(currentUser.id);
    await prefs.setString("user",jsonEncode(dataUser.toJson()));
    getSharedPreferences();
  }
